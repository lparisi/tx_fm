...
def load_json_files(file_paths):
    ...
    for file_path in file_paths:
        with open(file_path, 'r') as f:
            ...
            transactions2.extend(data if isinstance(data, list) else [data])
            transaction_len.append(len(data))
            #print(len(data))
    return transactions2,transaction_len
...
def tokenizer_file(window_range,token_max_for_each_transaction,seq_max_len,transaction_len,output_file_path, transactions, frequency_report, top_n_values, write_temp=False, temp_file=None):
    
    if write_temp and temp_file:
        temp_output_file = open(temp_file, 'w')
    
    global_cnt=0
    with open(output_file_path, 'w') as output_file:
        for length in tqdm(transaction_len, desc="Tokenizing transactions"):
            #print("=============================")
            ...
            for i in range(length):
                ...
                tokenized_transaction, log_messages_tokens = tokenize_transaction(transactions[global_cnt+i], frequency_report, top_n_values)
                tokenized_transactions.append(tokenized_transaction[:token_max_for_each_transaction])

                # Write log messages to the temp file if required
                if write_temp and temp_file:
                    temp_output_file.write(" ".join(log_messages_tokens) + "\n")
            global_cnt+=length
                        
            for i in range(0, length, window_range):
                concat_transaction=[]
                flag=0
                ...
                output_file.write(transaction_line)
            
                # Write numerical values to the temp file if required
                if write_temp and temp_file:
                    temp_output_file.write(" ".join(numerical_values) + "\n")
                    
                if flag==0:
                    break
    # Close the temp file if it was used
    if write_temp and temp_file:
        temp_output_file.close()

    print(f"Tokenized transactions have been saved to {output_file_path}")
    if write_temp and temp_file:
        print(f"Numerical values have been saved to {temp_file}")
    
if __name__ == '__main__':
    # Assuming you have a list of file paths
    input_dir = './../../../../sharedata/solana_multi_transaction_data/malicious_programs_filtered'
    frequency_file = '/home/lhhao0430/src_solana/datas/all_september_transaction_frequency_counter/frequency_counter.json'  
    output_file_path = '/home/lhhao0430/src_solana/datas/ma_data_nopad3_first/train_ma.txt'

    file_paths = glob.glob(input_dir + '/*.json')  
    transactions,transaction_len= load_json_files(file_paths)
    if not os.path.exists(frequency_file):
        frequency_counter = analyze_transactions(transactions)
    ...
    window_range = 9999999 # no sliding window
    token_max_for_each_transaction = 512
    seq_max_len = 4096*3

    temp_file = None
    write_temp = False
    tokenizer_file(window_range,token_max_for_each_transaction,seq_max_len,transaction_len,output_file_path, transactions, frequency_counter, top_n_values, write_temp=write_temp, temp_file=temp_file)
    # Running this file will generate the pre-tokenization data for all TXs saved in 'output_file_path'