...
def analyze_instruction(instruction, counters):
    ...
    if "logMessages" in instruction:
        for message in instruction["logMessages"]:
            cleaned_log_message = message.replace("\n", "<nl>")
            counters["logMessages"][cleaned_log_message] += 1
    ...
    for instruction in transaction.get('instructions', []):
        ...
        if parsed and not isinstance(parsed, str):
            ...
            for name, detail in parsed.get("info", {}).items():
                ...
                # round numeric (blockgpt)
                if is_numeric(data_token):
                    data_token = round_to_significant_figures(data_token, 2)
        ...
        for log_message in log_messages:
            ...
            # Check if the log message is among the top frequent or considered special
            if is_top_frequent(cleaned_log_message, "logMessages", frequency_report, top_n_values):
...
def tokenizer_file(window_range,token_max_for_each_transaction,seq_max_len,transaction_len,output_file_path, transactions, frequency_report, top_n_values, transaction_jsons, write_temp=False, temp_file=None):
    ...
    if write_temp and temp_file:
        temp_output_file = open(temp_file, 'a')
    ...
    #print(transaction_len)
    for i in range(transaction_len[1]):
        #print("++++++++++++++++++++++++++++++++++++")
        #print(type(transaction))
        #print(transaction)
        #transaction=transaction[0]
        tokenized_transaction = tokenize_transaction(transactions[i], frequency_report, top_n_values)
        #print(log_messages_tokens)
    ...
    for i in range(0, transaction_len[1], window_range):
        ...
        #print(len(concat_transaction))
        #print(concat_transaction[0:10])
        transaction_line = " ".join([str(token) for token in concat_transaction]) + "\n"
        #transaction_line = str(transaction_jsons)+" "+transaction_line
    ...
    return [output_file_path,transaction_lines,temp_file,numerical_values_lines]
...
if __name__ == '__main__':
    ...
    transactions1,transactions2,transaction_len,transaction_jsons= load_json_files(file_paths2)
    #print(transaction_len)
    ...
    output_file_path = '/home/lhhao0430/src_solana/datas/new_log_data/september/train.txt'
    temp_file = '/home/lhhao0430/src_solana/datas/new_log_data/september/temp.txt'
    ...
    for i in tqdm(range(len(transaction_len))):
        ...
        #tokenizer_file(9999999,512,4096,transaction_len,output_file_path, transactions, frequency_counter, top_n_values, transaction_jsons, write_temp=write_temp, temp_file=temp_file)
    #T2 = time.time() 
    pool.close()
    pool.join()
    T2 = time.time() 
